...
dbt_run_task = KubernetesPodOperator(  
    image="phong192016/my-dbt-project:v6",  # Image dbt từ Docker Hub
    cmds=["dbt"], 
    arguments=["run", "--profiles-dir", "/dbt", "--project-dir", "/dbt", "-s", "/dbt/models/example/*"],  # Thêm đầy đủ các arguments
    name="dbt-run-pod",
    task_id="dbt_run",
